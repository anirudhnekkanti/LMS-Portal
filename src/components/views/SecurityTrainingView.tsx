...
const SecurityTrainingView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await getCourses('security');
        if (response.success) {
          setCourses(response.courses);
        }
      } catch (error) {
        console.error('Error fetching security courses:', error);
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="md:flex md:items-center md:justify-between mb-8">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-gray-400 mt-2">
              Learn essential security practices and protocols
            </p>
          </div>
        </div>
        ...
        {courses.length === 0 && !isLoading && (
          <div className="text-center py-12">
            <p className="text-gray-400">No security courses available at the moment.</p>
          </div>
        )}
      </div>
    </div>
...
export default SecurityTrainingView;